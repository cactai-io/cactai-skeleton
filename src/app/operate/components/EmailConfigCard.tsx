// src/app/operate/components/EmailConfigCard.tsx
// Operator-panel UI for the invitation-email provider selection. Reads/
// writes project_state.decisions.invitation_email_v1 via the
// /api/operate/email-config endpoint (server route below).
//
// Provider options:
...
export function EmailConfigCard(): JSX.Element {
  ...
  useEffect(() => {
    void (async () => {
      try {
        const res = await lensFetch('/api/operate/email-config');
  ...
  const sendTest = async () => {
    ...
    setTestMsg(null);
    try {
      const res = await lensFetch('/api/operate/email-config/test', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
