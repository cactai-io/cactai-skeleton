// src/app/operate/components/TokenUsageCard.tsx
//
// v1.3.5 Build 3 — Token-usage display for the operate panel (R6).
  ...
  const reload = async (withBYOK: boolean) => {
    ...
    try {
      const res = await fetch(
        `/api/operate/token-usage${withBYOK ? '?include_byok=1' : ''}`,
        { cache: 'no-store' },
      );
