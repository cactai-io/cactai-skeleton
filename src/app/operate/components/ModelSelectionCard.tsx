// src/app/operate/components/ModelSelectionCard.tsx
// Operator-panel card wrapping the @cactai-io/mui ModelSelectionPanel
// (v1.3 Phase 14, Gap 81). Loads + saves model selections through the
// /api/operate/model-selections route; loads resolved model IDs via the
...
export function ModelSelectionCard(): JSX.Element {
  ...
  useEffect(() => {
    void (async () => {
      try {
        const res  = await lensFetch('/api/operate/model-selections');
  ...
  return (
    <div style={{
      ...
      <ModelSelectionPanel
        ...
        adminOnly={{}}
        onOperatorChange={() => {
          // Operator zone-level toggles are persisted alongside the
