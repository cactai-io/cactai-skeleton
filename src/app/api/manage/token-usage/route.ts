// src/app/api/manage/token-usage/route.ts
//
// v1.3.5 Build 3 — Management-panel token-usage proxy. Forwards to the
// platform's /v1/manage/token-usage endpoint with the deployed app's
...
export async function GET(req: NextRequest) {
  try {
    ...
    const res = await fetch(
      `${endpoints.cactaiBase}/v1/manage/token-usage${includeByok ? '?include_byok=1' : ''}`,
      {
