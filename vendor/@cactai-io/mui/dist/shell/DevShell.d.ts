...
export interface DevShellProps {
    ...
    /** Bumped by the host on every orchestrator turn so the live decision-log
     *  dock refetches as the developer confirms each step. The wizard advances
     *  many steps within a single build_phase, so workflowStep alone doesn't
